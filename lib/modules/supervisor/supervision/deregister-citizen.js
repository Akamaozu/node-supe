module.exports = function( supervisor, citizens, config ){
  ...
  function deregister_citizen( name ){
    ...
    // else stop then deregister
      ...
      supervisor.hook.add( name + '-stopped', 'complete-'+ name +'-deregistration', function(){
        supervisor.hook.del( name + '-stopped', 'complete-'+ name +'-deregistration' );
    ...
    function perform_deregister( name ) {
      ...
      supervisor.hook.run( 'citizen-deregistered', { name: name });
      supervisor.hook.run( name + '-deregistered' );
    }
  }
}
